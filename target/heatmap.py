...
def save_heatmaps(data, xnet, ynet,  timenet, every = 20, dpi = 350, out_file = 'times.txt'):
        
    b, a = np.meshgrid(xnet, ynet)
    ...
    for t in range(0, len(timenet), every):
        ...
        c = axes.contourf(a, b, data[t,:,:].T, cmap=cmap, levels = levels, vmin=l_c, vmax=r_c)
